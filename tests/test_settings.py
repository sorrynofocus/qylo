...
#
# Purpose:
# Pins the .env-derived settings in settings.py: the positive-int reader
# (positive_int_from_env, named etoi before the Phase C split) and provider resolution.
# A typo in .env must fail loudly rather than silently reverting to a default. Model-free.
#
# Run:
...
from qylo.settings import positive_int_from_env

CHUNK_SIZE = string_table.ENV_CHUNK_SIZE
PROVIDER = string_table.ENV_MODEL_PROVIDER


# --- positive_int_from_env: environment string to positive int ---------------


def test_unset_variable_falls_back_to_the_default(monkeypatch):
    monkeypatch.delenv(CHUNK_SIZE, raising=False)

    assert positive_int_from_env(CHUNK_SIZE, 1000) == 1000
...
def test_valid_value_is_returned_as_an_int(monkeypatch):
    ...
    assert positive_int_from_env(CHUNK_SIZE, 1000) == 512
...
def test_non_numeric_value_raises_rather_than_defaulting(monkeypatch):
    ...
    with pytest.raises(RuntimeError):
        positive_int_from_env(CHUNK_SIZE, 1000)
